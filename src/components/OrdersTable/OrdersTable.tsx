...
const OrdersTable = ({ data, loading, error, onUpdate }: OrdersTableProps) => {
  ...
  const columns: DataTableProps<Ticket>['columns'] = [
    ...
    },
    {
      ...
      render: (item: Ticket) => (
        editMode === item._id ? (
          <NumberInput
            value={editedRecord?.markedPrice || 0}
            onChange={(value) => setEditedRecord({ ...editedRecord, markedPrice: value })}
          />
        ) : (
    ...
    },
    {
      ...
      render: (item: Ticket) => (
        editMode === item._id ? (
          <NumberInput
            value={editedRecord?.price || 0}
            onChange={(value) => setEditedRecord({ ...editedRecord, price: value })}
          />
        ) : (
          <span>${item.markedPrice * (1 - item.discount)}</span>
        )
      ),
    },
    {
      ...
      render: (item: Ticket) => (
        editMode === item._id ? (
          <NumberInput
            value={editedRecord?.discount || 0}
            onChange={(value) => setEditedRecord({ ...editedRecord, discount: value })}
            min={0}
            max={1}
            precision={2}
            step={0.01}
    ...
    },
    {
      ...
      render: (item: Ticket) => (
        editMode === item._id ? (
          <DateTimePicker
            value={editedRecord?.discountExpiry ? new Date(editedRecord.discountExpiry) : null}
            onChange={(value) => setEditedRecord({ ...editedRecord, discountExpiry: value })}
          />
        ) : (
          <span>{item.discountExpiry ? new Date(item.discountExpiry).toLocaleDateString() : 'N/A'}</span>
        )
      ),
    },
    {
      ...
      render: (item: Ticket) => (
        editMode === item._id ? (
          <Select
            ...
            onChange={(value) => setEditedRecord({ ...editedRecord, status: value as TicketStatus })}
          />
        ) : (
    ...
    },
    {
      ...
      render: (item: Ticket) => (
        editMode === item._id ? (
          <NumberInput
            value={editedRecord?.sold || 0}
            onChange={(value) => setEditedRecord({ ...editedRecord, sold: value })}
            min={0}
          />
        ) : (
          <span>{item.sold}</span>
    ...
    },
    {
      accessor: 'remain',
      render: (item: Ticket) => (
        <span>{item.event?.capacity ? calculateRemainingTickets(item) : 'N/A'}</span>
      ),
      sortable: true,
    },
    {
    ...
    if (debouncedQuery || selectedStatuses.length) {
      filtered = data
        .filter(({ event, status }) => {
          if (
            debouncedQuery !== '' &&
            !event.eventName.toLowerCase().includes(debouncedQuery.trim().toLowerCase())
          ) {
            return false;
          }
