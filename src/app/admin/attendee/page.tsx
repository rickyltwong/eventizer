...
export default function AttendeeManagementPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/events')
      ...
      .then((data: Event[]) => {
        ...
        const types: string[] = Array.from(new Set(data.map((event: Event) => event.eventType)));
        setEventTypes(types);
      })
  ...
  const filteredAttendees = attendees.filter((attendee) => {
    const matchesSearchTerm =
      (attendee.name?.toLowerCase().includes(searchTerm.toLowerCase()) || 'unknown'.includes(searchTerm.toLowerCase())) ||
      (attendee.email?.toLowerCase().includes(searchTerm.toLowerCase()) || 'unknown'.includes(searchTerm.toLowerCase()));
    const matchesStatus =
      filterStatus === 'all' ||
    ...
    <Container size="lg" my="md">
      <Box style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <Group style={{ marginBottom: '2rem' }}>
          {eventTypes.map((type) => (
            <Button
              style={{ backgroundColor: '#63bac9', fontSize: '20px', padding: '10px 20px' }}
              key={type}
        ...
        <Box style={{ display: 'flex', width: '100%' }}>
          ...
          <Box style={{ width: '20%', paddingRight: '1rem' }}>
            <Title order={2} style={{ color: '#59B6C7', fontWeight: 'bold', textAlign: 'center', fontSize: '24px' }}>
              Events Names
            </Title>
            <Input
              type="text"
          ...
          <Box style={{ width: '80%' }}>
            ...
            <Group className="mb-4">
              <Box style={{ textAlign: 'center', marginRight: '20%', marginLeft: '30%' }}>
                ...
                >
                  <Text style={{ fontSize: '24px', fontWeight: 'bold' }}>{totalAttendees}</Text>
                </Box>
              </Box>
              <Box style={{ textAlign: 'center' }}>
                ...
                >
                  <Text style={{ fontSize: '24px', fontWeight: 'bold' }}>{checkedInAttendees}</Text>
                </Box>
              </Box>
            </Group>
