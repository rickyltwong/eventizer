...
export async function POST(request: NextRequest) {
  try {
    ...
    const { event, type, markedPrice, price, discount, discountExpiry, status, sold } = reqBody;
...
export async function PUT(req: NextRequest) {
  try {
    ...
    return new Response(JSON.stringify({ success: true, message: 'Ticket updated successfully', ticket: updatedTicket }), {
      status: 200,
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    return new Response(JSON.stringify({ success: false, message: 'Failed to update ticket' }), {
      status: 500,
      headers: {
        'Content-Type': 'application/json',
      },
