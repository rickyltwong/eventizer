...
const Dashboard = () => {
  ...
  async function fetchBarData() {
    try {
      ...
      const filteredEvents = eventData.filter(event => {
        ...
        const isTypeMatch = !selectedEventType || event.eventType === selectedEventType;
      ...
      setBarData({
        ...
        datasets: [
          ...
          {
            label: 'Remaining Seats',
            data: filteredEvents.map(event => Math.max(0, event.capacity - calculateSoldTickets(event, ticketData))),
            backgroundColor: 'rgba(75, 192, 192, 0.2)',
            borderColor: 'rgba(75, 192, 192, 1)',
            borderWidth: 1,
            barThickness: 15,
  ...
  return (
    <Container>
      ...
      <Group my="lg">
        {eventTypes.map((type) => (
          <Button key={type} onClick={() => setSelectedEventType(type)} style={{ backgroundColor: '#59B6C7', minWidth: '100px', height: '80px', fontSize: '18px', margin: '0 10px' }}>
            {type}
          </Button>
        ))}
        <Button onClick={() => setSelectedEventType(null)} style={{ backgroundColor: '#59B6C7', minWidth: '100px', height: '80px', fontSize: '18px', margin: '0 10px' }}>
          All
        </Button>
      </Group>
      ...
      <SimpleGrid cols={2} spacing="lg">
        <Card
          shadow="sm"
          p="lg"
          style={{ backgroundColor: '#f0f4f8', borderRadius: '12px', minHeight: '100px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}
        >
          <div>
            <Text size="lg" style={{ marginBottom: '8px', fontWeight: 'bold', textAlign: 'center' }}>
              Total Sales
            </Text>
            <Text size="xl" style={{ textAlign: 'center' }}>{totalSales}</Text>
          </div>
        </Card>
        <Card
          shadow="sm"
        ...
        >
          <div>
            ...
            <Text size="xl" style={{ textAlign: 'center' }}>{userCount}</Text>
          </div>
        </Card>
      </SimpleGrid>
